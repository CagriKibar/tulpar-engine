...
static void draw_stats_overlay(bool *open, float dt, const renderer::RendererStats &rs, const content::SceneDesc &scene,
  ...
  if (ImGui::Begin("##EngineStatsHUD", nullptr, wf)) {
    ...
    ImGui::TextColored(tone(Tone::Accent), "\xF0\x9F\x93\x8A"); // 📊
    ImGui::SameLine();
...
int editor_run(const EditorOptions &opts, const EditorHost *host) {
  ...
  while (running) {
    ...
    if (ImGui::Begin(kPanelGorunumLabel)) {
      ...
      if (ImGui::Button(" 🎬 Sahne ")) view_tab = ViewportTab::Scene;
      ...
      if (ImGui::Button(" 🎮 Oyun ")) view_tab = ViewportTab::Game;
      ...
      if (view_tab == ViewportTab::Scene) {
        ...
        ImGui::SameLine();
        if (ImGui::Button(wireframe_mode ? "▦ Tel Kafes" : "■ Düz Gölgeli")) wireframe_mode = !wireframe_mode;
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Gölgelendirme Modu");

        ImGui::SameLine();
        if (ImGui::Button("🎯 Odaklan (F)")) {
      ...
      if (ImGui::Button("\xF0\x9F\x93\x8A Stats")) show_stats = !show_stats;
      ImGui::PopStyleColor(3);
      if (ImGui::IsItemHovered()) ImGui::SetTooltip("Viewport İstatistikleri (FPS, Poligon, Bellek, Çöp)");
      ...
      if (ImGui::BeginPopup("ViewportOptionsMenu")) {
        if (ImGui::MenuItem("📊 Detaylı İstatistikler (Stats)", nullptr, show_stats)) show_stats = !show_stats;
        ImGui::Separator();
        if (ImGui::MenuItem("🎬 Sahne Görünümüne Geç", nullptr, view_tab == ViewportTab::Scene)) view_tab = ViewportTab::Scene;
        if (ImGui::MenuItem("🎮 Oyun Görünümüne Geç", nullptr, view_tab == ViewportTab::Game)) view_tab = ViewportTab::Game;
        ImGui::Separator();
        if (ImGui::MenuItem("📐 Izgarayı Göster / Gizle", nullptr, show_grid)) show_grid = !show_grid;
        ...
        if (ImGui::MenuItem("🎯 Seçili Varlığa Odaklan", "F")) {
          const int32_t s0 = st.sel.primary();
          if (s0 >= 0 && s0 < (int32_t)st.scene.entity_count) {
            static content::SceneBounds fb[content::kSceneMaxEntities];
            const uint32_t nb = entity_world_bounds(st, phys, fb);
        ...
        if (ImGui::MenuItem("🔄 Kamerayı Varsayılana Sıfırla")) {
    ...
    if (ImGui::Begin(kPanelSahneLabel)) {
      ...
      if (ImGui::BeginPopup("SahnePanelMenu")) {
        // --- 1. YENİ VARLIK EKLE ---
        if (ImGui::BeginMenu("\xEF\x80\xA7  Yeni Varl\xC4\xB1k Ekle...")) {
          ...
          if (ImGui::BeginMenu("\xE2\x98\x80  I\xC5\x9F\xC4\xB1k & Ayd\xC4\xB1nlatma")) {
            ...
            if (ImGui::MenuItem("\xE2\x9C\xA8  I\xC5\x9F\xC4\xB1k H\xC3\xBCzmeli G\xC3\xBCne\xC5\x9F (Sun + God Rays)")) do_add(19);
            ...
            if (ImGui::MenuItem("\xF0\x9F\x94\xA6  Hacimsel Spot I\xC5\x9F\xC4\xB1k (Volumetric)")) do_add(63);
            if (ImGui::MenuItem("\xE2\x9C\xA8  I\xC5\x9F\xC4\xB1k H\xC3\xBCzmesi (God Rays Shafts)")) do_add(64);
            ImGui::Separator();
        ...
        if (ImGui::BeginMenu("\xE2\x9A\xA1  H\xC4\xB1zl\xC4\xB1 \xC5\x9E" "ablonlar (Haz\xC4\xB1r Kurulum)")) {
          if (ImGui::MenuItem("\xF0\x9F\x8F\xB0  Fizik Test Odas\xC4\xB1 (Zemin + D\xC3\xBC\xC5\x9F" "enler)")) do_add(50);
          if (ImGui::MenuItem("\xF0\x9F\x8C\x85  Do\xC4\x9F" "a Paketi (G\xC3\xBCne\xC5\x9F + Skybox + Arazi + Su)")) do_add(51);
        ...
        if (ImGui::BeginMenu("\xF0\x9F\x93\xA6  Prefab \xC4\xB0\xC5\x9Flemleri")) {
          if (ImGui::MenuItem("\xF0\x9F\x93\x8E  Se\xC3\xA7ili Varl\xC4\xB1\xC4\x9F\xC4\xB1 Prefab Kaydet", nullptr, false, st.sel.count > 0)) {
            const int32_t s0 = st.sel.primary();
            if (s0 >= 0 && s0 < (int32_t)st.scene.entity_count) {
              const SceneEntity &sel_e = st.scene.entities[s0];
              char pf_path[128];
              std::snprintf(pf_path, sizeof(pf_path), "%s.prefab", sel_e.name);
              if (content::prefab_save(sel_e, pf_path)) {
                set_status(st, "Prefab kaydedildi: %s", pf_path);
              }
            }
          }
          if (ImGui::MenuItem("\xF0\x9F\x93\x82  Prefab Y\xC3\xBCkle ve Sahneye Ekle...")) {
            content::PrefabData pd{};
            if (content::prefab_load("SokakLambasi.prefab", pd) || content::prefab_load("Varlik.prefab", pd)) {
              int32_t new_idx = content::prefab_instantiate(st.scene, pd, Vec3{0, 0, 0});
              if (new_idx >= 0) {
                st.sel.clear();
                st.sel.toggle(new_idx);
                set_status(st, "Prefab sahneye eklendi: %s", pd.name);
              }
            } else {
              set_status(st, "Hata: Y\xC3\xBCklenecek .prefab dosyas\xC4\xB1 bulunamad\xC4\xB1.");
            }
          }
          ImGui::EndMenu();
        }
        ...
        if (ImGui::BeginMenu("\xF0\x9F\x9B\xA0  SROS \xE2\x80\x94 Sahne Onar\xC4\xB1m ve Bak\xC4\xB1m")) {
          ...
          if (ImGui::MenuItem("\xF0\x9F\x9B\xA1  Modellere Otomatik Collider Ekle")) {
            uint32_t added = 0;
            for (uint32_t i = 0; i < st.scene.entity_count; i++) {
              SceneEntity &e = st.scene.entities[i];
          ...
          if (ImGui::MenuItem("\xF0\x9F\xA7\xB9  T\xC3\xBCm Sahneyi Otomatik Onar (Quick Fix)")) {
            uint32_t ops = 0;
            for (uint32_t i = 0; i < st.scene.entity_count; i++) {
              SceneEntity &e = st.scene.entities[i];
              if (e.scale.x <= 0.001f || e.scale.y <= 0.001f || e.scale.z <= 0.001f) {
        ...
        // --- 5. AĞAÇ GÖRÜNÜMÜ ---
        if (ImGui::BeginMenu("\xF0\x9F\x93\x82  G\xC3\xB6r\xC3\xBCn\xC3\xBCm ve Katlama")) {
    ...
    if (ImGui::Begin(kPanelOzelliklerLabel)) {
      ...
      if (si >= 0 && si < (int)st.scene.entity_count) {
        ...
        if (ImGui::SmallButton(" \xE2\x8E\x98 \xC3\x87o\xC4\x9F" "alt ")) do_add(0); // 📋 Çoğalt (Ctrl+D)
        ImGui::SameLine();
        if (ImGui::SmallButton(" \xE2\x9C\x95 Sil ")) do_remove(); // ✕ Sil (Del)
        ImGui::SameLine();
        if (ImGui::SmallButton(" \xE2\x8C\x96 Odaklan ")) cam.target = e.pos; // ⌖ Odaklan (F)
        ImGui::PopStyleColor(2);
        ImGui::PopStyleVar();
        ...
        if (has_l) {
          ...
          if (begin_component_card("\xE2\x98\x80", "I\xC5\x9F\xC4\xB1k", content::kSceneLight, nullptr, &act, true, Tone::Warn)) {
            if (prop_begin("isik")) {
              ...
              ImGui::Text("\xE2\x9C\xA8  I\xC5\x9F\xC4\xB1k H\xC3\xBCzmesi (God Rays / Volumetric)");
              ImGui::PopStyleColor();
        ...
        // 3. Bilgilendirici İpucu Kartı (Kırpılmayan esnek şerit)
        {
          ...
          if (ImGui::BeginChild("##scene_hint_card", ImVec2(hint_w, 0), ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_NoScrollbar)) {
            ImGui::TextColored(tone_col(Tone::Accent), "\xF0\x9F\x92\xA1 \xC4\xB0" "pucu:");
            ImGui::SameLine();
    ...
    // Sahnedeki engellerin (duvar, kutu, vb.) gunesi fiziksel olarak perdelemesi
    // ve arkasindan gercek isik saftlari akmasi icin sahne derinlik testine tabi
    ...
    if (st.scene.godrays_enabled && gr_intensity > 0.0f && st.prims[content::kPrimSphere].valid()) {
      if (gr_source.w == 0.0f) {
        ...
        // Nokta veya Spot Isik (Point / Spot Light)
